					...
					<form action="searchVm" method="post">
						<div class="content-search-condition pull-left">
							<div class="search-condition">
								<span>名称</span> <input id="condition-name" name="condition-name"
									type="text" />
							</div>
						</div>
						<div class="content-search-condition pull-left">
							<div class="search-condition">
								...
								<div class="input-group"
									...
									<input type="text" id="selected-state" name="selected-state"
										readonly="readonly" data-toggle="dropdown"
										class="form-control dropdown-toggle" style="font-size: 18px;">
									<div class="input-group-btn">
										<button type="button"
											class="btn btn-default dropdown-toggle show-state"
						...
						<div class="content-search-condition pull-left">
							<div class="search-condition">
								...
								<div class="input-group"
									style="width: 200px; float: left; margin-left: 20px;">
									<input type="text" id="selected-os" name="selected-os"
				...
				<div class="user">
					<div class="user-operate">
						<button id="add-vm" type="button" class="btn btn-default">
							添加虚拟机</button>
						<button id="delete-vm" type="button" disabled="disabled"
							title="只能对状态为“不可用”的系统虚拟机或者状态为“关闭”|“不可用”的用户虚拟机执行此操作"
							class="btn btn-default">删除</button>
						<button id="launch-vm" type="button" disabled="disabled"
							title="只能对状态为“关闭”的用户虚拟机执行此操作" class="btn btn-default">
							启动</button>
						<button id="restart-vm" type="button" disabled="disabled"
							title="只能对状态为“启动”的用户虚拟机执行此操作" class="btn btn-default">
							重启</button>
						<button id="close-vm" type="button" disabled="disabled"
							title="只能对状态为“启动”的用户虚拟机执行此操作" class="btn btn-default">
							关闭</button>
						<button id="edit-vm" type="button" disabled="disabled"
							title="只能对“关闭”状态的用户虚拟机执行此操作，一次只能修改一台" class="btn btn-default">
							修改配置</button>
					</div>
								...
								<tbody class="data-table-tbody">
									<c:forEach var="vmNeedInfo" items="${vmNeedInfos }">
										<tr vid="${vmNeedInfo.vmInstance.id }">
											...
											<td><span style="line-height: 50px;"
												status="${vmNeedInfo.vmInstance.status }"> <c:if
														test="${vmNeedInfo.vmInstance.powerStatus == 'Running'}">启动</c:if>
													<c:if
														test="${vmNeedInfo.vmInstance.powerStatus == 'Halted'}">关闭</c:if>
													<c:if
														test="${vmNeedInfo.vmInstance.powerStatus == 'Creating'}">创建中</c:if>
													<c:if
														test="${vmNeedInfo.vmInstance.powerStatus == 'Closing'}">关闭中</c:if>
													<c:if
														test="${vmNeedInfo.vmInstance.powerStatus == 'Restarting'}">重启中</c:if>
													<c:if
					...
					function() {
						...
						if (vmCpuUsedRate) {
							...
							setCircleProgressColorInRange($(this).find(
									".vm-cpu-used-rate"), vmCpuUsedRate);
						}
						if (vmMemoryUsedRate) {
							vmMemoryUsedRate = vmMemoryUsedRate.split("%")[0];
							setCircleProgressColorInRange($(this).find(
									".vm-memory-used-rate"), vmMemoryUsedRate);
						}
					});
		});
	</script>
